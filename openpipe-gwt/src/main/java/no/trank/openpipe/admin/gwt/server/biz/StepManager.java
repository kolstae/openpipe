...
public class StepManager {
         ...
         for(String className : pipelineStepClassNames) {
            ...
            for(PropertyDescriptor propDesc : Introspector.getBeanInfo(clazz).getPropertyDescriptors()) {
               if(propDesc.getWriteMethod() != null) {
                  try {
                     fields.put(propDesc.getName(), getType(clazz, propDesc.getName()));
                  }
   ...
   private Type getType(Class clazz, String propertyName) throws NoSuchFieldException {
      try {
      ...
      return clazz.getSuperclass() != null ? getType(clazz.getSuperclass(), propertyName) : null;
   }
